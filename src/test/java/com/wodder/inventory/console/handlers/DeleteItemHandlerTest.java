...
class DeleteItemHandlerTest extends BaseMenuTest {
	...
	@Captor
	private ArgumentCaptor<InventoryItemDto> captor;
	...
	void handleInput(Long id) {
		when(model.deleteItem(any(InventoryItemDto.class))).thenReturn(new Result<>(Boolean.TRUE, null));
		setChars("id=" + id);
	...
	void printErrorOnDelete(Long id) {
		when(model.deleteItem(any(InventoryItemDto.class))).thenReturn(new Result<>(null, "Unable to delete item"));
		setChars("id=" + id);
		deleteItemHandler.handleInput(in, out, err);
